...
import copy
from torch.utils.data import DataLoader, IterableDataset, TensorDataset, Dataset
...
def main(data_root, ont, model_name, batch_size, epochs, load, alpha_test, combine, device, run):
    ...
    wandb_logger = wandb.init(project="dgpu-combined", name= model_name, group= f"mlp-{ont}")
    ...
    terms_dict, train_data, valid_data, test_data, test_df, go_embeds = load_data(data_root, ont)
    n_terms = len(terms_dict)

    
    net = DGPROModel(n_terms, device).to(device)
    go_embeds = go_embeds.to(device)
    wandb.watch(net)
    ...
    # train_dataset = GODataset(train_features, train_labels, go_embeds)
    # valid_dataset = GODataset(valid_features, valid_labels, go_embeds)
    # test_dataset = GODataset(test_features, test_labels, go_embeds)
    ...
    if not load:
        ...
        for epoch in range(epochs):
            net.train()
            train_loss = 0
            train_steps = len(train_loader)#int(math.ceil(len(train_labels) / batch_size))
            with ck.progressbar(length=train_steps, show_pos=True) as bar:
                for batch_features, batch_labels in train_loader:
                    ...
                    batch_features = th.cat([batch_features.repeat_interleave(len(go_embeds), dim=0),
                                             go_embeds.repeat(bs, 1)], dim=1)
                    batch_labels = batch_labels.reshape(-1, 1)
                    
                    # bs, num_gos, embed_dim = batch_features.shape
                    # batch_features = batch_features.view(bs * num_gos, embed_dim)
                    # batch_labels = batch_labels.view(bs * num_gos, 1)
            ...
            with th.no_grad():
                ...
                with ck.progressbar(length=valid_steps, show_pos=True) as bar:
                    for batch_features, batch_labels in valid_loader:
                        ...
                        logits = logits.reshape(bs, len(go_embeds))
...
class DGPROModel(nn.Module):
    ...
    def __init__(self, nb_gos, device, nodes=[2048,]):
        super().__init__()
        self.nb_gos = nb_gos
...
def load_data(data_root, ont):
    ...
    return terms_dict, train_data, valid_data, test_data, test_df, go_embeds
...
class GODataset(Dataset):
    def __init__(self, prot_data, labels, go_embeds):
        self.prot_data = prot_data
        self.labels = labels
        self.go_embeds = th.FloatTensor(go_embeds)
        
        prot_ids = list(range(len(self.prot_data)))
        go_ids = list(range(len(self.go_embeds)))
        # self.go_id_to_name = dict(zip(go_ids, self.go_embeds.keys()))
        # self.data_samples = np.array(list(product(prot_ids, go_ids)))

    def __len__(self):
        return len(self.prot_data)
        return len(self.data_samples)

    def __getitem__(self, idx):

        prot_embed = self.prot_data[idx].repeat(len(self.go_embeds), 1)
        embeds = th.cat([prot_embed, self.go_embeds], dim=1)

        labels = self.labels[idx]
        
        # prot_id, go_id = self.data_samples[idx]
        # prot_data = self.prot_data[prot_id]
        # go_embed = self.go_embeds[self.go_id_to_name[go_id]]
        # data = th.cat([prot_data, go_embed])
        # label = self.labels[prot_id, go_id]
